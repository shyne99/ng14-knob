import {
  Component, Input, Output, EventEmitter, ViewChild, ElementRef, forwardRef, AfterViewInit
} from '@angular/core';
...
providers: [{
    ...
    useExisting: forwardRef(() => Ng14KnobComponent),
    multi: true
  }]
})
...
/**
 * @class
 * @name Ng14KnobComponent
 * @description a knob component based on https://github.com/joedotjs/ngKnob/blob/master/dist/ngKnob.js
 */
export class Ng14KnobComponent implements ControlValueAccessor, AfterViewInit {
  /** the minimum possible value */
    ...
    @Output('valueChange') valueChange: EventEmitter<number> = new EventEmitter<number>(false);
  ...
  /** the current rotation persisted (not persisted until the touch/mouse down is released) */
    ...
    ngAfterViewInit() {
        this.knobDiv.nativeElement.addEventListener('mousedown',  (e: any) => { this.setChangeListener(e); });
        this.knobDiv.nativeElement.addEventListener('touchstart',  (e: any) => { this.setChangeListener(e); });
        this.calculateDialAngle();
    }
    ngAfterContentInit() {}
    ...
    calculateChange(mousePositionChange: number) {
        ...
        if (this.intensive) {
            this.valueChange.emit(this.meterValue);
            this.notifyChange();
        }
    }
    ...
    setChangeListener(e: any) {
        ...
        var funcRemove = function (e: any) {
            if (!self.intensive) {
                self.valueChange.emit(self.meterValue);
                self.notifyChange();
            }
            if (e.stopPropagation) e.stopPropagation();
            if (e.preventDefault) e.preventDefault();
